#!/usr/bin/env python3
"""Formal deterministic game runner for werewolf-agent-skills.

The runner is production-facing orchestration code: it wires the programmatic
judge, game-resolution core, ACL channels, protocol validators, and a pluggable
player responder into one complete 6-player game loop. Test modules should call
this runner instead of owning gameplay flow themselves.
"""
...
def main() -> None:
    """CLI entrypoint for a single deterministic 6-player game."""
    parser = argparse.ArgumentParser(description="Run one deterministic 6-player werewolf game")
    parser.add_argument("--seed", type=int, default=7)
    parser.add_argument("--max-days", type=int, default=6)
    parser.add_argument("--state-file", default=None)
    args = parser.parse_args()

    result = simulate_game_6p(seed=args.seed, max_days=args.max_days, state_file=args.state_file)
